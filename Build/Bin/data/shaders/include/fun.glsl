...
mat4 GetBillboardMatrix(const mat4 model, const mat4 view, float billboardEnabled, float lockX, float lockY) {
    
    // if billboarding is completely disabled, return original model matrix
    if (billboardEnabled == 0.0 || lockX == 1.0 && lockY == 1.0) {
        return model;
    }
    ...
    if (lockX == 1.0) {
        ...
        vec3 cameraRight = vec3(1.0, 0.0, 0.0); // fixed right vector (X-axis locked)
    ...
    if (lockY == 1.0) {
        ...
        vec3 cameraUp = vec3(0.0, 1.0, 0.0); // fixed up vector (Y-axis locked)
